using System;
using System.Collections.Generic;
using System.Text;
using Xamarin.Forms;

namespace gigIt.Xamarin
{
    public static class gigItIcons
    {
        public const string Work = "\ue001";
        public const string Tasks = "\ue002";
        public const string Actors = "\ue003";
        public const string Forecast = "\ue00a";
        public const string Revisions = "\ue00b";
        public const string News = "\ue00f";
        public const string Skills = "\ue010";
        public const string Calendar = "\ue012";
        public const string Loading = "\ue013";
        public const string Email = "\ue014";
        public const string Phone = "\ue015";
        public const string Chat = "\ue016";
        public const string Video = "\ue017";
        public const string TextSms = "\ue018";
        public const string Pencil = "\ue01a";
        public const string Adjust = "\ue01b";
        public const string Wrench = "\ue01c";
        public const string UserFollow = "\ue01d";
        public const string UserConnect = "\ue01e";
        public const string UserMessage = "\ue01f";
        public const string QuestionFore = "\ue020";
        public const string QuestionBack = "\ue019";
        public const string InfoFore = "\ue021";
        public const string InfoBack = "\ue022";
        public const string Pin1 = "\ue02b";
        public const string KeySecure3 = "\ue02c";
        public const string Filter3 = "\ue00c";
        public const string CloseButton2 = "\ue004";
        public const string MinusButton3 = "\ue005";
        public const string Attention1 = "\ue006";
        public const string AddButton3 = "\ue007";
        public const string CloseOff5 = "\ue008";
        public const string Check5 = "\ue000";
        public const string List = "\ue009";
        public const string Ideas = "\ue00d";
        public const string Campfire = "\ue00e";
        public const string Learn = "\ue011";
        public const string Connect = "\ue024";
        public const string Fire1 = "\ue02d";
        public const string Fire2 = "\ue02e";
        public const string Fire3 = "\ue02f";
        public const string Rocket = "\ue030";
        public const string Search = "\ue023";
        public const string Followers = "\ue025";
        public const string Speaker = "\ue026";
        public const string Thoughts = "\ue027";
        public const string Market = "\ue028";
        public const string Torch = "\ue029";
        public const string Spark = "\ue02a";
        public const string Gigit = "\ue031";
        public const string People = "\ue032";
        public const string Close = "\ue033";
    }

}
